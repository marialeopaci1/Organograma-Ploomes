...
with col_main:
    ...
    if is_full:
        df_view = df
        repulsao = -55000 # Aumentado para dar espaço aos cards 10x maiores
        spring = 800      # Aumentado para as linhas não ficarem curtas
    else:
    ...
    for _, row in df_view.iterrows():
        ...
        # AQUI ESTÁ A MUDANÇA: Aumentado em 10x
        if is_ceo:
            t_f, m_i, l_m, b = 280, 120, 1400, 18 
        else:
            t_f, m_i, l_m, b = 150, 70, 750, 5    
    ...
    edges = [{"from": r["LIDER_NORM"], "to": r["NOME_NORM"], "arrows": "to", "color": "#000000", "width": 5} 
    ...
    <div id="mynetwork" style="height: 820px; background: white; border-radius:15px; border: 1px solid #ddd;"></div>
    ...
    <script>
        ...
        network.once('stabilized', function() {{ 
            ...
            if(sN !== "{normalizar_nome('Nenhum selecionado')}") network.focus(sN, {{ scale: 0.2, animation: true }});
            document.getElementById('loading').style.display = 'none'; 
        }});
        
        setTimeout(() => {{ document.getElementById('loading').style.display = 'none'; }}, 12000);
    </script>
    <style>@keyframes spin {{ 0% {{ transform: rotate(0deg); }} 100% {{ transform: rotate(360deg); }} }}</style>
    """
    components.html(html_vis, height=840)
